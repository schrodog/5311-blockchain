...
class Block:
  ...
  def genesis(self):
    ...
    self.prev_hash = 0x0
    self.current_hash = 0x000dc75a315c77a1f9c98fb6247d03dd18ac52632d7dc6a9920261d8109b37cf
    self.nonce = 0
    self.transaction = []
...
class Blockchain:    
  def __init__(self):
    #genesis = Block({'timestamp': time.time(), 'prev_hash': 0x0, 'current_hash': 0x000dc75a315c77a1f9c98fb6247d03dd18ac52632d7dc6a9920261d8109b37cf, 'nonce': 0, 'transaction':[], 'merkle_root': [], 'index': 0})
  ...
  def calculate_hash(self, index, previousHash, timestamp, data, nonce):
    s = sha256()
    s.update((str(timestamp)[0:10]).encode())
    return s.hexdigest()
  
  def mine(self, data):
    new_block = self.generate_next_block(data)
  ...
  def generate_next_block(self, data):
    ...
    timestamp = time.time()
    nonce = 0
    next_hash = self.calculate_hash(next_index, previous_hash, timestamp, data, nonce)
